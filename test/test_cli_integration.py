...
from converter.utils import generate_output_filename, validate_dataframe
...
class TestCLIIntegration(unittest.TestCase):
    ...
    def test_revolut_csv_conversion(self, mock_read_input):
        ...
        self.assertAlmostEqual(float(output_df.iloc[0]['Amount']), -4.5, places=2)
        self.assertEqual(output_df.iloc[1]['Payee'], 'From John')
        self.assertAlmostEqual(float(output_df.iloc[1]['Amount']), 50.0, places=2)
    ...
    def test_nbg_account_conversion(self, mock_read_input):
        ...
        self.assertAlmostEqual(float(output_df.iloc[0]['Amount']), -45.67, places=2)
        self.assertEqual(output_df.iloc[1]['Date'], '2025-07-14')
        self.assertEqual(output_df.iloc[1]['Payee'], 'JOHN DOE')
        self.assertAlmostEqual(float(output_df.iloc[1]['Amount']), 1500.00, places=2)
    ...
    def test_exclude_previous_transactions(self):
        ...
        self.assertTrue(os.path.exists(self.revolut_csv), "Input test file should exist")
        self.assertTrue(os.path.exists(self.previous_ynab_csv), "Previous YNAB file should exist")
        
        # Read the previous YNAB CSV to verify its contents
        prev_df = pd.read_csv(self.previous_ynab_csv)
        self.assertEqual(len(prev_df), 1, "Previous YNAB file should have 1 row")
        self.assertEqual(prev_df.iloc[0]['Payee'], 'Coffee Shop', "Previous transaction payee should match")
        self.assertAlmostEqual(float(prev_df.iloc[0]['Amount']), -4.5, places=2, msg="Previous transaction amount should match")
