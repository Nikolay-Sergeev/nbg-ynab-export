...
class TestPerformance(unittest.TestCase):
    ...
    def _generate_revolut_data(self, num_rows):
        ...
        for t in types:
            if t == 'CARD_PAYMENT':
                ...
                fee = (-np.random.uniform(0, 2)
                       if np.random.random() < 0.3 else 0)
            else:
                amount = (np.random.uniform(10, 1000) if t == 'TRANSFER'
                          else -np.random.uniform(10, 500))
                fee = 0
    ...
    def _generate_account_data(self, num_rows):
        ...
        return pd.DataFrame({
            ...
            'Περιγραφή': [
                f"Transaction {i}" for i in range(num_rows)
            ],
    ...
    def _generate_card_data(self, num_rows):
        ...
        for _ in range(num_rows):
            if np.random.random() < 0.3:
                ...
                descriptions.append(
                    f"3D SECURE E-COMMERCE ΑΓΟΡΑ - "
                    f"Merchant {np.random.randint(1, 1000)}"
                )
            else:
                descriptions.append(
                    f"Regular Merchant {np.random.randint(1, 1000)}"
                )

        # Create random amounts
        debit_credit = np.random.choice(
            ['Χ', 'Π'], size=num_rows, p=[0.8, 0.2]
        )
        amounts = []
        for dc in debit_credit:
            if dc == 'Χ':
    ...
    def test_revolut_processing_performance(self):
        ...
        self.assertLess(
            duration, 2.0,
            f"Revolut processing took too long: {duration:.2f}s"
        )
        print(f"Revolut processing of {len(self.large_revolut_df)} rows "
    ...
    def test_account_processing_performance(self):
        ...
        self.assertLess(
            duration, 2.0,
            f"Account processing took too long: {duration:.2f}s"
        )
        print(f"Account processing of {len(self.large_account_df)} rows "
              f"took {duration:.2f}s")
    ...
    def test_card_processing_performance(self):
        ...
        self.assertLess(
            duration, 2.0,
            f"Card processing took too long: {duration:.2f}s"
        )
        print(f"Card processing of {len(self.large_card_df)} rows "
              f"took {duration:.2f}s")
    ...
    def test_exclusion_performance(self):
        ...
        self.assertLessEqual(len(result_df), len(self.large_new_df))

        # Performance assertion - should process quickly
        self.assertLess(
            duration, 2.0,
            f"Transaction exclusion took too long: {duration:.2f}s"
        )
        print(
            f"Exclusion of {len(self.large_prev_df)} transactions from "
            f"{len(self.large_new_df)} took {duration:.2f}s"
        )
    ...
    def test_amount_conversion_performance(self):
        ...
        amount_strings = [
            f"{np.random.uniform(1, 1000):.2f}".replace('.', ',')
        ...
        self.assertLess(
            duration, 1.0,
            f"Amount conversion took too long: {duration:.2f}s"
        )
        print(f"Conversion of {len(amount_strings)} amounts "
              f"took {duration:.2f}s")
