...
class ActualClient:
    ...
    def __init__(self, base_url: str, password: str):
        ...
        try:
            resp = self.session.post(f"{self.base_url}/api/login", json={"password": self.password}, timeout=10)
    ...
    def get_budgets(self) -> list:
        ...
        try:
            ...
            resp = self.session.get(f"{self.base_url}/api/budgets", timeout=10)
            if resp.status_code == 401:
                resp = self.session.post(f"{self.base_url}/api/budgets", json=self._auth_params(), timeout=10)
            ...
            logging.error("Actual get_budgets failed: %s", e)
            raise
    ...
    def get_accounts(self, budget_id: str) -> list:
        ...
        try:
            ...
            logging.error("Actual get_accounts failed: %s", e)
            raise
    ...
    def get_transactions(self, budget_id: str, account_id: str, count: int = None, page: int = None, since_date: str = None) -> list:
        ...
        try:
            ...
            logging.error("Actual get_transactions failed: %s", e)
            raise
    ...
    def upload_transactions(self, budget_id: str, transactions: list) -> dict:
        ...
        try:
            ...
            logging.error("Actual upload_transactions failed: %s", e)
            raise
